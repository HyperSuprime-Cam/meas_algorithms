...
class SourceDetectionConfig(pexConfig.Config):
    ...
    tempLocalBackground = pexConfig.ConfigurableField(
        doc=("A local (small-scale) background estimation and removal before footprint and peak detection. "
             "It is added back into the image after detection. The purpose is to suppress the "
             "detection of peaks in the wings of bright objects, which are caused by noise."),
        target=SubtractBackgroundTask,
    )
    doTempLocalBackground = pexConfig.Field(
        dtype=bool,
        doc="Do temporary local (small-scale) background subtraction before footprint detection?",
        default=True,
    )
